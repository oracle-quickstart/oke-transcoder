resource "oci_core_vcn" "tc_vcn" {
  count = var.useExistingVcn ? 0 : 1
  cidr_block     = var.VCN_CIDR
  compartment_id = var.compartment_ocid
  display_name   = "OKE-TC-VCN - ${random_string.deploy_id.result}" 
  dns_label      = var.vcn_dns_label
}

resource "oci_core_internet_gateway" "tc_internet_gateway" {
  count = var.useExistingVcn ? 0 : 1
  compartment_id = var.compartment_ocid
  display_name   = "tc_internet_gateway"
...
resource "oci_core_service_gateway" "tc_service_gateway" {
  count = var.useExistingVcn ? 0 : 1
  compartment_id = var.compartment_ocid
  services {
...
resource "oci_core_route_table" "public" {
  ...
  route_rules {
    ...
    network_entity_id = oci_core_internet_gateway.tc_internet_gateway.*.id[count.index]
  }
}
...
resource "oci_core_route_table" "private" {
  ...
  route_rules {
      ...
      network_entity_id = oci_core_service_gateway.tc_service_gateway.*.id[count.index]
    }
...
resource "oci_core_subnet" "edge" {
  ...
  compartment_id      = var.compartment_ocid
  vcn_id              = var.useExistingVcn ? var.custom_vcn[0] : oci_core_vcn.tc_vcn.0.id
  route_table_id      = oci_core_route_table.public[count.index].id
  security_list_ids   = [oci_core_security_list.EdgeSubnet.*.id[count.index]]
  dhcp_options_id     = oci_core_vcn.tc_vcn[count.index].default_dhcp_options_id
  dns_label           = "edge"
}
